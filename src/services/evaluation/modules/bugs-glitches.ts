...
export class BugsGlitchesModule extends BaseEvaluationModule {
  readonly moduleId = "bugs_glitches" as const;
  readonly moduleName = "Bugs & Glitches";
  readonly description = "Detects potential bugs, runtime errors, edge cases, race conditions, and logic flaws in the codebase";
  readonly version = "1.0";

  async evaluate(input: ModuleInput): Promise<ModuleResult> { return this.buildResult(input); }

  private ctx(input: ModuleInput) {
    const all = getAllEvaluationModules();
    return getAnalysisForModule(this.moduleId, all.map((m) => ({ id: m.moduleId, name: m.moduleName, description: m.description })), {
